...
use crate::{
    drivers::fs::core::{FileSystem, STORAGE_CONTROLLERS},
    horse_lib::{bytes::bytes2str, fd::Path},
};
...
impl FAT {
    ...
    fn get_cluster_offset(&self, cluster: u32) -> u32 {
        let sector_num = self.bpb.rsvd_sec_cnt as u32
            + self.bpb.num_fats as u32 * self.bpb.fatsz32
            + (cluster - 2) * self.bpb.sec_per_clus as u32;
    ...
    fn read_file(&self, path: &Path, buf: &mut [u8], nbytes: usize) -> isize {
        ...
        while cluster != END_OF_CLUSTER_CHAIN {
            ...
            if self.bpc * (i + 1) <= bytes_to_read {
                ...
                buf[self.bpc * i..bytes_to_read]
                    .copy_from_slice(&bytes_buf[..(bytes_to_read - self.bpc * i)]);
                break;
            }
