...
impl<const N: usize> Allocator<N> {
    ...
    pub fn alloc_mem(&mut self, size: usize, alignment: usize) -> Option<NonNull<[u8]>> {
        self.initialize();
        let mut ptr = Self::ceil(self.current, alignment);
        let next_boundary = Self::ceil(self.current, self.boundary);
    ...
    pub fn alloc_array<T: 'static>(&mut self, len: usize) -> Option<NonNull<[T]>> {
