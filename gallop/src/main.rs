...
fn test_null() {
    ...
    let fd = match open("/dev/null", OpenFlags::RDWR) {
        ...
        Err(e) => {
            fail!("open /dev/null", "open failed");
            eprintln!("  err: {:?}", e);
            return;
        }
    };
...
fn test_zero() {
    ...
    let fd = match open("/dev/zero", OpenFlags::RDWR) {
        ...
        Err(e) => {
            fail!("open /dev/zero", "open failed");
            eprintln!("  err: {:?}", e);
            return;
        }
    };
    ...
    let n = read(fd, &mut buf);
    check!(
        "read /dev/zero returns 8 bytes",
        n == Ok(8),
        "expected Ok(8)"
    );
    check!(
        "read /dev/zero fills zeros",
        buf.iter().all(|&b| b == 0),
        "buffer not all zero"
    );
...
fn test_stdin() {
    ...
    let fd = match open("/dev/stdin", OpenFlags::RDONLY) {
        ...
        Err(e) => {
            fail!("open /dev/stdin", "open failed");
            eprintln!("  err: {:?}", e);
            return;
        }
    };
    ...
    let n = read(fd, &mut buf);
    check!(
        "read /dev/stdin (not implemented -> EOF)",
        n == Ok(0),
        "expected Ok(0)"
    );
    ...
    let r = write(fd, b"x");
    check!(
        "write to /dev/stdin returns Err(Inval)",
        r == Err(Error::Inval),
        "expected Err(Inval)"
    );
...
fn test_stdout() {
    ...
    let fd = match open("/dev/stdout", OpenFlags::WRONLY) {
        Ok(fd) => {
            pass!("open /dev/stdout");
            fd
        }
        Err(e) => {
            fail!("open /dev/stdout", "open failed");
            eprintln!("  err: {:?}", e);
            return;
        }
    };
    ...
    check!(
        "read from /dev/stdout returns Err(Inval)",
        r == Err(Error::Inval),
        "expected Err(Inval)"
    );
...
fn test_stderr() {
    ...
    let fd = match open("/dev/stderr", OpenFlags::WRONLY) {
        Ok(fd) => {
            pass!("open /dev/stderr");
            fd
        }
        Err(e) => {
            fail!("open /dev/stderr", "open failed");
            eprintln!("  err: {:?}", e);
            return;
        }
    };
...
pub extern "C" fn _start() -> ! {
    println!("========================================");
    println!("  Gallop - initial process for HorseOS");
    println!("========================================");
